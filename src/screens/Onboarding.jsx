        ...
        <div className="flex-1 px-6 pt-8 overflow-y-auto animate-fade-in">
          ...
          <div className="bg-white rounded-card card-shadow overflow-hidden">
            {['breakfast', 'lunch', 'dinner'].map((meal, idx) => (
              <div
                key={meal}
                className={`flex items-center gap-3 pl-4 pr-3 min-h-[64px] ${
                  idx < 2 ? 'border-b border-line' : ''
                }`}
              >
                <span className="flex-1 min-w-0 text-[16px] font-medium">
                  {MEAL_LABELS[meal]}
                </span>
                <input
                  type="time"
                  ...
                  className="w-[92px] shrink-0 rounded-lg border border-line bg-cream px-2 py-1.5 text-[14px] tabular-nums disabled:opacity-40 focus:outline-none focus:border-sage"
                />
                <Switch
